...
import numpy, random, operator, json
...
#load CityList from json
def loadCityList():
    #open file
    file = open('CityList.json',)
    data = json.load(file)
    #get CityList data from file
    a = data['CityList']
    noCities = len(a)
    coords = []
    cityList = []
    #loop to get cityList
    for x in range(noCities):
        b = a[x]
        coords.append(str(b.get("x")) + "," + str(b.get("y")))
        splitco = coords[x].split(",")
        cityList.append(City(x=int(splitco[0]), y=int(splitco[1])))
    file.close
    return cityList
